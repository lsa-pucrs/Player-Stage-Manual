      ...
      RangerProxy &sp)
{
	  ...
	  // Bail on bad sonar reads
	  if (sp.GetRangeCount()<4) {
		  printf("only %d sonar ranges?\n",sp.GetRangeCount());
	  	  return;
	  }
...
*/
int FindItem(item_t *itemList, int listLength, SimulationProxy &sim, char *robot_name)
{
	/*
	...
	*/
	...
	//radius of the search circle
      ...
      //find the robot...
      sim.GetPose2d(robot_name, robotX, robotY, robotYaw);
...
{	
      ...
      p2dProxy2.RequestGeom();
      sonarProxy2.RequestGeom();
      sonarProxy2.RequestConfigure();
      ...
      {		
            ...
            if(laserProxy2.GetRangeCount() >= 90 && laserProxy2[90] < 0.25)
            {
                  int destroyThis;
            ...
            //set motors
            p2dProxy2.SetSpeed(forwardSpeed2, dtor(turnSpeed2));
            sleep(1);
      }
}
